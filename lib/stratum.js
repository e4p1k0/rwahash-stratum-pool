...
var StratumClient = function(options){
    ...
    function handleAuthorize(message, replyToSocket){
        _this.fullLogin = message.params[0];
        _this.workerPass = message.params[1];

        const arr = message.params[0].split('.');
        _this.login = arr[0];
        _this.worker = arr[1] ? arr[1] : 'default';

        options.authorizeFn(_this.remoteAddress, options.socket.localPort, _this.login, _this.workerPass, function(result) {
    ...
    this.getLabel = function(){
        return (_this.login || '(unauthorized)') + ' [' + _this.remoteAddress + ']';
    };
...
var StratumServer = exports.Server = function StratumServer(options, authorizeFn){
    ...
    this.manuallyAddStratumClient = function(clientObj) {
        ...
        if (subId != null) { // not banned!
            stratumClients[subId].manuallyAuthClient(clientObj.login, clientObj.workerPass);
