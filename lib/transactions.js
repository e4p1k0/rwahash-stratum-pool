...
if (rpcData.foundation) {
    if (rpcData.foundation[0]?.payee) {
        var payeeReward = 0;

        payeeReward = rpcData.foundation[0].amount;
        reward -= payeeReward;
        rewardToPool -= payeeReward;
	
        var payeeScript = util.addressToScript(rpcData.foundation[0].payee);
        txOutputBuffers.push(Buffer.concat([
            util.packInt64LE(payeeReward),
            util.varIntBuffer(payeeScript.length),
            payeeScript
    ...
    if (rpcData.foundation[1]?.payee) {
        var payeeReward = 0;

        payeeReward = rpcData.foundation[1].amount;
        reward -= payeeReward;
        rewardToPool -= payeeReward;

        var payeeScript = util.addressToScript(rpcData.foundation[1].payee);
        txOutputBuffers.push(Buffer.concat([
            util.packInt64LE(payeeReward),
            util.varIntBuffer(payeeScript.length),
            payeeScript
...
exports.CreateGeneration = function(rpcData, publicKey, extraNoncePlaceholder, reward, txMessages, recipients){
    ...
    var txVersion = txMessages === true ? 6 : 8;	
    var txLockTime = 0;
